...
/** 最近写入/替换过的会话（供指针失效时渲染回退，修复“卡住/冻结”）。 */
let lastCommittedSession = "";
...
export function replaceState(sessionId: string, next: TaskState): void {
	...
	lastCommittedSession = sessionId;
}
...
export function getRenderState(): TaskState {
	return slotFor(resolveRenderSession());
}

/**
 * 解析渲染应读的会话：优先前台指针；若指针指向已驱逐/未知会话（会话替换/压缩后
 * 指针未前移的“卡住”场景），回退到最近写入的会话，避免 overlay 冻结或渲染空。
 */
function resolveRenderSession(): string {
	if (activeRenderSession !== "" && sessions.has(activeRenderSession)) return activeRenderSession;
	return sessions.has(lastCommittedSession) ? lastCommittedSession : "";
}
